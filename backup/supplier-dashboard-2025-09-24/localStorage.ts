...
export const seedMarketData = () => {
  // إضافة بيانات تجريبية للسوق إذا لم تكن موجودة
  if (storage.getMarketItems().length === 0) {
  ...
  // إضافة بيانات تجريبية لطلبات التجار
  if (storage.getMerchantOrders().length === 0) {
  ...
  // إضافة بيانات تجريبية لخدمات الشحن
  if (storage.getShippingServices().length === 0) {
