...
// طلبات الشحن - منفصلة عن طلبات السوق
export interface ShippingOrder {
  ...
  // خيارات النشر
  publishToMarketplace?: boolean; // هل تم نشره في السوق المشترك
  publishScope?: 'عام' | 'خاص'; // نطاق النشر (عام لكل شركات الشحن أو خاص للشركاء فقط)
  publishedAt?: string | null; // تاريخ النشر في السوق
}
...
export const storage = {
  ...
  // طلبات الشحن
  getShippingOrders: (): ShippingOrder[] => {
    const shippingOrders = localStorage.getItem('business_shipping_orders');
    return shippingOrders ? JSON.parse(shippingOrders) : [];
  },

  addShippingOrder: (order: Omit<ShippingOrder, 'id' | 'createdAt' | 'status'>): ShippingOrder => {
    const shippingOrders = storage.getShippingOrders();
    ...
    localStorage.setItem('business_shipping_orders', JSON.stringify(shippingOrders));
    try {
      console.debug('[storage] addShippingOrder -> dispatching shipping-orders-updated', newOrder);
      window.dispatchEvent(new CustomEvent('shipping-orders-updated', { detail: { order: newOrder } }));
    } catch (e) {
      // ignore in non-window environments
    }
    return newOrder;
  ...
  updateShippingOrder: (id: string, updates: Partial<ShippingOrder>): ShippingOrder | null => {
    const shippingOrders = storage.getShippingOrders();
    const index = shippingOrders.findIndex(o => o.id === id);
    if (index === -1) return null;
    
    shippingOrders[index] = { ...shippingOrders[index], ...updates };
    localStorage.setItem('business_shipping_orders', JSON.stringify(shippingOrders));
    try {
      window.dispatchEvent(new CustomEvent('shipping-orders-updated', { detail: { order: shippingOrders[index] } }));
    } catch (e) {
      // ignore
    }
    return shippingOrders[index];
  },

  deleteShippingOrder: (id: string): boolean => {
    const shippingOrders = storage.getShippingOrders();
    const filtered = shippingOrders.filter(o => o.id !== id);
    localStorage.setItem('business_shipping_orders', JSON.stringify(filtered));
    try {
      window.dispatchEvent(new CustomEvent('shipping-orders-updated'));
    } catch (e) {
      // ignore
    }
    return filtered.length !== shippingOrders.length;
  },
